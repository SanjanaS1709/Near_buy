...
import 'package:nearbuy_app/screens/shop_dashboard_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _login() async {
    ...
    try {
      final result = await _apiService.login(_emailController.text.trim(), _passwordController.text.trim(), _selectedRole);
      if (result.containsKey('access_token')) {
        final role = result['role'];
        
        if (role?.toString().toLowerCase() == 'shop_owner') {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Login Successful (Shop Owner)")));
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => ShopDashboardScreen(ownerId: result['user_id'])),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Login Successful (Customer)")));
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => CustomerHomeScreen(userId: result['user_id'])),
          );
        }
      } else {
